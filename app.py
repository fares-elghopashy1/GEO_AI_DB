...
def load_data():
    ...
    geo = gpd.read_file("grid_features.geojson")
...
with tab1:
    ...
    # Project for a stable map center (centroid in lon/lat is misleading on EPSG:4326).
    _proj = top3.to_crs(3857)
    _c_ll = gpd.GeoSeries(_proj.geometry.centroid, crs=_proj.crs).to_crs(4326).iloc[0]
    m = folium.Map(
        location=[_c_ll.y, _c_ll.x],
        zoom_start=13,
